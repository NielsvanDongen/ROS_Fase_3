...
class move_to_part_belt_right_armSM(Behavior):
	...
	def create(self):
		...
		_state_machine = OperatableStateMachine(outcomes=['finished', 'failed', 'unkown_id'], input_keys=['part_type', 'pose'], output_keys=['part_type_r'])
		...
		_state_machine.userdata.config_name = 'beltPreGrasp'
		...
		with _state_machine:
			...
			OperatableStateMachine.add('ReplacePartType',
										...
										remapping={'value': 'part_type', 'result': 'part_type_r'})
